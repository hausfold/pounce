...
# container, CI) boot a bare Linux box with no Nix. Pounce is consumed as a
# flake (`packages.pounce`, the nebelung palette input that generates
# `Palette+nebelung.generated.swift`), so without Nix a session can't re-lock or
