...
|---|---|---|
| `AGENTS.md` | Codex, OpenCode, Cursor, Zed, Amp, Copilot-in-editor, and anything else that speaks [agents.md](https://agents.md) | **The source of truth.** Every project rule, starting with the routing rule that keeps launch/signing/theming changes out of this repo. |
...
belong to the workshop. When pounce grows one of its own it goes in
`.agents/skills/<name>/SKILL.md`, symlinked into `.claude/skills/<name>/` and
`.opencode/skills/`, never copied.

## Caveats

- **Pounce is a macOS app; a Linux cloud container can't build it.** The build
  shells out to `/usr/bin/xcrun swiftc` against system frameworks, and
  `tests/run.sh` compiles Swift the same way. The bootstrap is there so a cloud
  session can resolve and re-lock the flake — not so it can produce a
  `Pounce.app`.
