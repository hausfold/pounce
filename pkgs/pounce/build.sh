...
#   POUNCE_SKILL_MD                    the agent skill to embed (`pounce skill`).
#                                      Defaults to ../../ai/SKILL.md, which is
#                                      where it is committed; Nix passes a store
#                                      path because the repo root is outside its
#                                      `src = ./.` sandbox.
...
# The agent skill, embedded in the binary (`pounce skill`). The family standard
# requires embedding rather than a file on disk: a cask ships an .app, a Nix
# build a store path, a tarball a folder — only this is uniform across all
# three, and it is what makes the binary that answers `--help` the binary that
# answers `skill`.
#
# ai/SKILL.md lives at the REPO root, outside the Nix derivation's
# `src = ./.` (pkgs/pounce alone) — the same sandbox boundary that keeps
# ../assets and ../pounce-commands out of reach. So Nix passes the store path in
# POUNCE_SKILL_MD and every other packager gets the relative default; both then
# run this identical generator, which is what keeps the embedded bytes the same
# bytes `pkgs/pounce-skill` installs.
SKILL_MD="${POUNCE_SKILL_MD:-../../ai/SKILL.md}"
if [ ! -f "$SKILL_MD" ]; then
  echo "build.sh: no agent skill at $SKILL_MD." >&2
  echo "  It is committed at ai/SKILL.md; point POUNCE_SKILL_MD at it if you" >&2
...
if grep -qF '"""#' "$SKILL_MD"; then
  echo "build.sh: $SKILL_MD contains the raw-string terminator \"\"\"# — it cannot be" >&2
  echo "  embedded as-is. Reword that line, or widen the delimiter here and in Skill.swift." >&2
  exit 1
fi
{
  echo "// AUTO-GENERATED by build.sh from ai/SKILL.md — do not edit or commit."
  echo 'let pounceSkillMarkdown = #"""'
  cat "$SKILL_MD"
  # A file not ending in a newline would weld the delimiter onto its last line.
  if [ -n "$(tail -c1 "$SKILL_MD")" ]; then echo; fi
  echo '"""#'
} > Skill.generated.swift
