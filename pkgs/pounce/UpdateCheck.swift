...
final class UpdateNudge {
    ...
    // The nudge's banner. The notification surface itself is Banner.swift —
    // trill when this Mac has it, Apple's own when it doesn't — and
    // `pounce.update` is what a `~/.config/trill/rules.json` rule matches on,
    // so "stop telling me about pounce updates" is a rule rather than a switch
    // nobody shipped.
    private static func postBanner(version: String, kind: InstallKind) {
        Banner.post(title: "Pounce \(version) is out", body: kind.bannerHint,
                    source: "pounce.update", symbol: "arrow.down.circle")
    }
}
