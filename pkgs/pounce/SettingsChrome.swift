...
// The settings-window chrome, shared by shape with perch and trill: a sidebar
// of panes, and each pane a scrolling column of cards. Deliberately a copy
// rather than a shared package — it is a couple of hundred lines of plain
// SwiftUI, and those two build through Xcode while pounce builds through a bare
// `swiftc` invocation in Nix, so a dependency would cost more than the
// duplication does. Keep the three in step by eye; if that stops working, that
// is when it earns a package.
//
// COPIED VERBATIM from trill's Trill/UI/SettingsChrome.swift apart from this
// note. If you change a shape here, change it there and in perch too, or the
// family stops looking like one family. What pounce adds on top of these pieces
// — the controls that write config.json — is SettingsControls.swift, kept
// separate so this file stays diffable against the other two.
//
// Note what this file does NOT use: `Theme.current`, pounce's own palette. The
// palette is the LAUNCHER's costume, themable per user and per appearance; a
// settings window is a mac settings window and should look like every other one
// on the machine. So this is system colours throughout, and it follows macOS
// light/dark rather than config.json's "theme".
        ...
        ScrollView(.vertical) {
            VStack(alignment: .leading, spacing: 18) {
                VStack(alignment: .leading, spacing: 3) {
                    Text(title)
                        .font(uiFont(19, weight: .semibold))
                    Text(subtitle)
